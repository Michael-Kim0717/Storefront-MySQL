            ...
            if (isNaN(response.quantity)){
                ...
                connection.query("SELECT * FROM products WHERE item_id = " + item_id, function(error, results){
                    if (parseInt(response.quantity) > parseInt(JSON.stringify(results[0].stock_quantity))){
                        console.log("Insufficient Quantity! Please select a quantity less than or equal to " + results[0].stock_quantity + ".\n");
                        askCustomerForQuantity(item_id);
                    }
                    else {
                        console.log();
    ...
    function confirmPurchase(results, quantity) {
        inquirer.prompt([
            ...
            if (response.confirm == "Yes"){
                console.log("Thank you for your purchase! :) Here is an update on your item. \n");
                connection.query("UPDATE products SET product_sales = " + (results[0].price * quantity + results[0].product_sales) + " WHERE item_id = " + results[0].item_id);
                connection.query("UPDATE products SET stock_quantity = " + (results[0].stock_quantity - quantity) + " WHERE item_id = " + results[0].item_id);
                connection.query("SELECT * FROM products WHERE item_id = " + results[0].item_id, function(error, response){
                    ...
                    console.log(
                        "| " + 
                        JSON.stringify(response[0].item_id).padEnd(9, " ") + " | " +
                        JSON.stringify(response[0].product_name).padEnd(30, " ") + " | " + 
                        JSON.stringify(response[0].department_name).padEnd(31) + " | " + 
                        "$" + JSON.stringify(response[0].price).padEnd(8) + " | " +
                        JSON.stringify(response[0].stock_quantity).padEnd(14) + " | " +
                        "$" + JSON.stringify(response[0].product_sales).padEnd(12) + " |"
                    );
                    console.log("".padEnd(125, "-"));
                    purchaseAgain();
                });
